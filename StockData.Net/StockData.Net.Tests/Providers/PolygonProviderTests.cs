...
{
    private IPolygonClient _client = null!;
    private ILogger<PolygonProvider> _logger = null!;
    private PolygonProvider _provider = null!;

    [TestInitialize]
    public void Setup()
    {
        _client = Substitute.For<IPolygonClient>();
        _logger = Substitute.For<ILogger<PolygonProvider>>();
        _provider = new PolygonProvider(_client, _logger);
    }

    [TestMethod]
    public void Constructor_NullClient_ThrowsArgumentNullException()
    {
        Assert.ThrowsExactly<ArgumentNullException>(() => _ = new PolygonProvider(null!, _logger));
    }

    [TestMethod]
    public void Constructor_NullLogger_ThrowsArgumentNullException()
    {
        Assert.ThrowsExactly<ArgumentNullException>(() => _ = new PolygonProvider(_client, null!));
    }
    ...
    public async Task GetStockInfoAsync_ValidQuote_ReturnsSerializedPayload()
    {
        _client.GetQuoteAsync("AAPL", Arg.Any<CancellationToken>())
            .Returns(new PolygonQuote(188.42, 1_700_000_000));
    ...
    {
        _client.GetQuoteAsync("AAPL", Arg.Any<CancellationToken>()).Returns((PolygonQuote?)null);

        var ex = await Assert.ThrowsExactlyAsync<InvalidOperationException>(() => _provider.GetStockInfoAsync("AAPL"));
        StringAssert.Contains(ex.Message, "Polygon GetStockInfoAsync failed");
    }
    ...
    public async Task GetHistoricalPricesAsync_ValidResponse_MapsCandlesToJson()
    {
        _client.GetHistoricalPricesAsync("MSFT", Arg.Any<DateTime>(), Arg.Any<DateTime>(), Arg.Any<CancellationToken>())
            .Returns([
                new PolygonAggregateBar(1_700_000_000, 10, 11, 9, 10.5, 1000),
                new PolygonAggregateBar(1_700_086_400, 10.5, 12, 10, 11.5, 1200)
            ]);
        ...
        Assert.AreEqual("polygon", doc.RootElement[0].GetProperty("SourceProvider").GetString());
    }
    ...
    [TestMethod]
    public async Task GetNewsAsync_NewsItems_ReturnsFormattedNewsBlock()
    {
        _client.GetNewsAsync("NVDA", Arg.Any<DateTime>(), Arg.Any<DateTime>(), Arg.Any<CancellationToken>())
            .Returns([
                new PolygonNewsItem("id-1", "Headline", "Reuters", "https://example.com/a", "Summary", DateTimeOffset.Parse("2026-01-03T14:10:00Z"), ["NVDA", "QQQ"])
            ]);
        ...
        StringAssert.Contains(result, "Related Tickers: NVDA, QQQ");
    }
    ...
    {
        _client.GetQuoteAsync("AAPL", Arg.Any<CancellationToken>())
            .Returns(new PolygonQuote(1.0, 1_700_000_000));
    ...
    {
        _client.GetQuoteAsync("AAPL", Arg.Any<CancellationToken>())
            .Returns<Task<PolygonQuote?>>(x => throw new HttpRequestException("upstream down"));
    ...
    {
        _client.GetQuoteAsync("AAPL", Arg.Any<CancellationToken>())
            .Returns<Task<PolygonQuote?>>(x => throw new OperationCanceledException("cancelled"));
    ...
    {
        _client.GetQuoteAsync("AAPL", Arg.Any<CancellationToken>())
            .Returns<Task<PolygonQuote?>>(x => throw new InvalidOperationException("token ABCD1234EFGH5678 should not leak"));
    ...
    {
        var ex = await Assert.ThrowsExactlyAsync<InvalidOperationException>(() => _provider.GetStockInfoAsync("ABCDEFGHIJKLMNOPQRSTU"));
        Assert.IsInstanceOfType<ArgumentException>(ex.InnerException);
    }
    ...
    public async Task GetStockInfoAsync_CaretPrefixTicker_IsAllowed()
    {
        _client.GetQuoteAsync("^VIX", Arg.Any<CancellationToken>())
            .Returns(new PolygonQuote(20.5, 1_700_000_001));
